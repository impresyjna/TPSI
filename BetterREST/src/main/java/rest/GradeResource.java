...
public class GradeResource {
    ...
    public List<Grade> getStudents(@PathParam("index") final long index, @PathParam("courseId") final long courseId) {
    ...
    @Path("/{gradeId}")
    @Produces({MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML})
    public Response getOneStudent(@PathParam("index") final long index, @PathParam("courseId") final long courseId, @PathParam("gradeId") final int gradeId) {
    ...
    @Produces({MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML})
    public Response createGrade(@PathParam("index") final long index, @PathParam("courseId") final int courseId, @Valid Grade grade) {
    ...
    @Consumes({MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML})
    @Produces({MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML})
    public Response updateGrade(@PathParam("index") final long index, @PathParam("courseId") final int courseId, @Valid Grade grade) {
        if (grade.validateNote()) {
            Course choosenCourse = null;
            for (Course course : Model.getInstance().getCourses()) {
                if (course.getId() == courseId) {
            ...
            for (int i = 0; i < choosenCourse.getGrades().size(); i++) {
                ...
                if (tempGrade.getId() == grade.getId()) {
                    if (grade.getStudent() == null) {
    ...
    public Response deleteStudent(@PathParam("index") final long index, @PathParam("courseId") final long courseId, @PathParam("gradeId") final int gradeId) {
